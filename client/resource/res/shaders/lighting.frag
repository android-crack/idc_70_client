...
vec3 computeLighting(vec3 normalVector, vec3 lightDirection, vec3 lightColor, float intensity, float specularIntensity, float attenuation)
{
    float diffuse = max(dot(normalVector, lightDirection), 0.0);
     vec3 diffuseColor = intensity*lightColor * _baseColor.rgb * diffuse * attenuation * 0.7;
    ...
    vec3 specularColor = vec3(pow(specularAngle, u_specularExponent)) * attenuation*specularIntensity;
...
vec3 getLitPixel()
{
    ...
    vec3 ambientColor = _baseColor.rgb * u_ambientColor * 0.8;
    ...
    for (int i = 0; i < DIRECTIONAL_LIGHT_COUNT; ++i)
    {
        ...
        #endif 
        combinedColor += computeLighting(normalVector, -lightDirection, u_directionalLightColor[i],
										u_directionalLightIntensity[i],u_specularIntensity[i], 1.0);
    }
    #endif
