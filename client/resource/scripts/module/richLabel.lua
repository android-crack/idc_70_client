...
-- local imageStr = "[image=res/ui/port/goods/port_cargo_6.png scale = 1.2][/image]" --创建文件名为test.png的精灵，大小为1.2
-- local multiStr = "[fontColor=42426f]哈哈哈哈哈哈!![/fontColor][res/ui/port/goods/port_cargo_6.png scale=1.3][/image]"
local ChineseSize = 3 -- 修正宽度缺陷(范围:3~4)
local RichLabel = class("RichLabel", function()
	local layer = CCLayerRGBA:create()
...
function RichLabel:ctor(param)
	param.str = param.str or T("传入的字符为nil")
	...
	-- 大小
	self.size = CCSize(param.rowWidth, ocHeight + useHeight + param.rowSpace)
end
...
function RichLabel:setAnchorPointExt(point)
	self:ignoreAnchorPointForPosition(false)
	self:setContentSize(self:getSize())
...
function RichLabel:tab_createButton(params)
	...
	params.text = string.trim(params.text)
	...
	local label = ui.newTTFLabel({
            ...
            color = params.color,
            font  = params.font,
			--dimension = CCSize(params.width, params.height),
			x = params.x,
			y = params.y
        })
 
    local useWidth = label:getContentSize().width
    local useHeight = label:getContentSize().height
	
    return label,useWidth,useHeight
end
...
--			end
	...
	-- 普通格式label显示
	if table.nums(clumpheadTab) == 0 or not tStr then
		local ptab = {}
		ptab.text = str
		if param then
			param.number = 1
