...
local ClsWealthRankCell = class("ClsWealthRankCell", ClsScrollViewItem)
...
ClsWealthRankCell.updateUI = function(self, cell_data, panel)
...
ClsWealthRankCell.mkUi = function(self)
	...
	self.coin_num:setText(self.m_data.value)
end

ClsWealthRankCell.onTap = function(self, x,y)
	if self.m_data.uid == getGameData():getPlayerData():getUid() then
		getUIManager():create("gameobj/playerRole/clsRoleInfoView")
		return
	end

	local main_ui = getUIManager():get("ClsRankMainUI"):getListView(WEALTH_TYPE)
	main_ui:openExpandPanel(self)
end

local ClsWealthRankUI = class("ClsWealthRankUI", function() 
	return UIWidget:create() 
end)

ClsWealthRankUI.ctor = function(self, type)
...
ClsWealthRankUI.initUi = function(self)
	self.panel = GUIReader:shareReader():widgetFromJsonFile("json/rank_wealth.json")
...
ClsWealthRankUI.updateView = function(self)
	...
	self.my_wealth_num:setText(getGameData():getPlayerData():getCash())
	...
	self.m_list_view = ClsScrollView.new(self.m_list_width, self.m_list_height, true, function()
		local cell_ui = GUIReader:shareReader():widgetFromJsonFile("json/rank_wealth_list.json")
		return cell_ui
	end)
	for i,v in ipairs(wealth_rank_info.rank_list) do
		v.pos = i
		self.cells[i] = ClsWealthRankCell.new(CCSize(770, 75), v)
	end
...
ClsWealthRankUI.closeExpandPanel = function(self)
	getUIManager():close("ClsRankExpandPop")
end

ClsWealthRankUI.openExpandPanel = function(self, select_cell)
