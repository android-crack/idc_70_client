...
local ClsExploreFishEvent = class("ClsExploreFishEvent", ClsExploreEventBase)

function ClsExploreFishEvent:initEvent(event_date)
	...
	self.m_auto_speed = event_config_item.auto_speed
	self.m_skill_id = self.m_active_skill_item.skill_info_id
	self.m_stop_reason_shake = string.format("%s_ExploreFishEvent_id%s_sceneShake", self.m_event_type, tostring(self.m_eid))
	self.m_stop_reason_be_attack = string.format("%s_ExploreFishEvent_id%s_moster_attack", self.m_event_type, tostring(self.m_eid))
	self.m_stop_reason_my_attack = string.format("%s_ExploreFishEvent_id%s_my_attack", self.m_event_type, tostring(self.m_eid))
	self.m_stop_reason_wait = string.format("%s_ExploreFireEvent_id%s_wait_1s", self.m_event_type, tostring(self.m_eid))
	...
	self.m_is_playing_attack_ani = false
	...
	self.m_moster_touch_time = 0
	if self.m_event_type == "monster" then
		self.m_is_moster = true
	else
		self.m_is_moster = false
	end
...
function ClsExploreFishEvent:touch(touch_reason_str)
	self.m_delay_timer = nil
	if self.m_is_firing or self.m_is_lock_touch or self.m_is_end then
		self.m_ships_layer:releaseStopShipReason(self.m_stop_reason_my_attack)
		return
	end
	...
	if self:isInDistance(self.m_skill_radius, x, y, px, py) then
		self.m_ships_layer:setStopShipReason(self.m_stop_reason_my_attack)
...
function ClsExploreFishEvent:updateDelayCallback(dt)
...
function ClsExploreFishEvent:update(dt)
	...
	if dis2 <= (self.m_hit_radius*self.m_hit_radius) then
		if not self.m_is_hit then
			self:autoForwardUpdate(dt)
			self:hit()
			if not self.m_is_moster then
				self.m_is_end = true
			end
	...
	if self.m_is_moster and self.m_is_hit then
		self.m_moster_touch_time = self.m_moster_touch_time + dt
		if self.m_moster_touch_time >= 5 then
			self:subSailor(self.m_passive_skill_item.params, nil, 4)
			self.m_moster_touch_time = 0
		end
	end
	self:updateAnimation(dis2)
	
	if has_key then
		if not self.m_event_layer:hasActiveKey(self.m_active_key) then
			self.m_event_layer:addActiveKey(self.m_active_key, function() return self:getQteBtn(self.m_stop_reason_wait, 1) end)
		end
	else
...
--播放动作
function ClsExploreFishEvent:updateAnimation(dis2)
	local play_radius2 = self.m_hit_radius * self.m_hit_radius * 4
	if not self.m_is_moster then
		if dis2 < play_radius2 then
			if not self.m_is_playing_attack_ani then
				self.m_is_playing_attack_ani = true
				...
				if not self.m_item_model:animationIsPlaying(self.m_event_config_item.animation_res[2]) then
					self.m_item_model:playAnimation(self.m_event_config_item.animation_res[1], true, true)
				end
			end
		else
			self.m_is_playing_attack_ani = false
		...
		if dis2 < play_radius2 then
			...
			self.m_is_playing_attack_ani = false
			self.m_item_model:stopAnimation(self.m_event_config_item.animation_res[3])
			self.m_item_model:playAnimation(self.m_event_config_item.animation_res[1], true, true)
		end
	end
end

function ClsExploreFishEvent:hit() --船撞到了
	self.m_is_hit = true
	if self.m_is_moster then
		self.m_ships_layer:setStopShipReason(self.m_stop_reason_be_attack)
	end
	local function began_callback()
		audioExt.playEffect(music_info[self.m_event_config_item.hit_sound].res)
		self.m_ships_layer:setStopShipReason(self.m_stop_reason_shake)

		if self.m_is_moster then
			local function callback()
				self:sendRemoveEvent(self.m_hit_not_remove_flag)
			end
			self.timer = scheduler:scheduleScriptFunc(callback,5,false)
		else
			self:sendRemoveEvent(self.m_hit_remove_flag)
			local rate_n = self.m_passive_skill_item.params  --百分比
	...
	local function end_callback()
		self.m_ships_layer:releaseStopShipReason(self.m_stop_reason_shake)
	end
...
function ClsExploreFishEvent:showEventEffect()
	...
	local function eff_action()
		...
		local hit_value = 1
		local add_value = ClsSkillCalc.NonBattleCalc[ClsSkillCalc.EXPLORE_REMOVE_NET_MONSTER](self.m_hp) --是否这次直接成功
		if add_value > 0 then
			hit_value = add_value
		end
		...
		local hit_sound = self.m_active_skill_item.hit_sound
		if self.m_is_moster then
			local hit_sound_tab = string.split(hit_sound, ",")
			hit_sound = hit_sound_tab[math.random(1,#hit_sound_tab)]
		end
		audioExt.playEffect(music_info[hit_sound].res)
	...
	local ClsExploreNetSkill = require("gameobj/explore/exploreNetSkill")
	local param = {
		skill_id = self.m_skill_id,
		targetNode = self.m_item_model.node,
		ship = self.m_player_ship,
		targetData = {spItem = self.m_item_model},
		num = 1,
		targetCallBack = eff_action,
		modelFile = "ex_net",
		animationFile = "ex_net",
	}
	self.m_bullet = ClsExploreNetSkill.new(param)
	audioExt.playEffect(music_info[self.m_active_skill_item.fire_sound].res)
end

function ClsExploreFishEvent:release()
	self.m_ships_layer:releaseStopShipReason(self.m_stop_reason_shake)
	self.m_ships_layer:releaseStopShipReason(self.m_stop_reason_my_attack)
	self.m_ships_layer:releaseStopShipReason(self.m_stop_reason_be_attack)
	self.m_event_layer:removeActiveKey(self.m_active_key)
	...
	if self.m_item_model then
		...
		if self.m_is_moster and self.m_is_hit then
			if getGameData():getSupplyData():getCurSailor() <= 0 then
				getGameData():getPortData():setEnterPortCallBack(function()
					self:showDialogTips(5, nil, 6)
				end)
			end
		end
	end
