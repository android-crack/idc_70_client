...
function colorsRGB.decToHex(IN)
...
function colorsRGB.correctColor(c,k)
	if c[k]<=0 then c[k]=0 end
	if c[k]>=255 then c[k]=255 end
end

function colorsRGB.RGBToHex(c)
	colorsRGB.correctColor(c,"R")
	colorsRGB.correctColor(c,"G")
	colorsRGB.correctColor(c,"B")
    local output = "0xff"..colorsRGB.decToHex(c["R"]) .. colorsRGB.decToHex(c["G"]) .. colorsRGB.decToHex(c["B"]);
    return tonumber(output)
end

function colorsRGB.ARGBToHex(c)
    local output = "0x"..colorsRGB.decToHex(c["A"])..colorsRGB.decToHex(c["R"]) .. colorsRGB.decToHex(c["G"]) .. colorsRGB.decToHex(c["B"]);
    return tonumber(output)
end
