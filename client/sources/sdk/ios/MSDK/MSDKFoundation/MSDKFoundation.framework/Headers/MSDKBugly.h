...
#pragma mark -
#pragma mark - MSDKBuglyDelegate

@protocol MSDKBuglyDelegate <NSObject>
@optional
/**
...
#pragma mark -
#pragma mark - MSDKBuglyConfig

@interface MSDKBuglyConfig : NSObject

/**
 *    @brief  delegate
 */
@property (nonatomic, assign, nullable) id<MSDKBuglyDelegate> delegate;
...
/**
 *    @brief 设置是否启用非正常退出事件上报，默认为NO
 */
@property (nonatomic, assign) BOOL enableUnexpectedTerminatingDetection;
...
typedef NS_ENUM(NSUInteger, MBLYLogLevel) {
    MBLYLogLevelSilent  = 0,
    MBLYLogLevelError   = 1 << 0, // 00001
    MBLYLogLevelWarn    = 1 << 1, // 00010
    MBLYLogLevelInfo    = 1 << 2, // 00100
    MBLYLogLevelDebug   = 1 << 3, // 01000
    MBLYLogLevelVerbose = 1 << 4, // 10000
};

#pragma mark - MSDKBugly
 ...
 */
+ (void)initLogger:(MBLYLogLevel) level consolePrint:(BOOL) printConsole;
 ...
 *    @param fmt   日志内容
 */
+ (void)level:(MBLYLogLevel) level logs:(nonnull NSString *)message;
 ...
 *    @param fmt   日志内容
 */
+ (void)level:(MBLYLogLevel) level log:(nonnull NSString *)format, ...;
 ...
 *    @param format 日志内容
 */
+ (void)level:(MBLYLogLevel)level tag:(nonnull NSString *) tag log:(nonnull NSString *)format, ...;

/**
 *    @brief  初始化MSDKBugly崩溃上报，在application:didFinishLaunchingWithOptions:方法中调用
 *
...
/**
 *    @brief 初始化MSDKBugly，并设置默认配置，默认配置内容可以查看MSDKBuglyConfig了解
 *
 *    @param appId 应用的唯一标识，在Bugly网站注册应用后获取
 *    @param defaultConfig 默认配置
 */
+ (void)startWithAppId:(nonnull NSString *)appId
                config:(nullable MSDKBuglyConfig *)config;

/**
 *    @brief 初始化MSDKBugly，并设置AppGroups标识(如果需要支持App Extension异常上报，请调用此接口初始化)
 *
 *    @param appId 应用的唯一标识，在Bugly网站注册应用后获取
 *    @param defaultConfig 默认配置
 ...
 *    @param key
 */
+ (nullable NSString *)sceneValueForKey:(nonnull NSString *)key;

/**
 *    @brief 获取所有自定义数据,Key-Value为NSString
 */
 ...
 *    @param reason 异常原因
 *    @param dict   附带数据,Key-Value为NSString
 */
 ...
 *    @param reason 异常原因
 *    @param stack  异常堆栈
 *    @param dict   附加数据,Key-Value为NSString
 ...
 *  设置SDK的唯一标识及SDK的版本信息
 *
 *  @param componentId   组件模块唯一标识
 *  @param version 版本信息
 */
+ (void)setComponentIdentifier:(nonnull NSString *) componentId version:(nonnull NSString *)version;
