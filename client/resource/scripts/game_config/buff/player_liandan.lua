...
cls_player_liandan = class("cls_player_liandan", clsBuffBase);

-- 属性段
---------------------------------------------------------------

-- 状态ID 
cls_player_liandan.get_status_id = function(self)
	return "player_liandan";
end


-- 状态名 
cls_player_liandan.get_status_name = function(self)
	return T("玩家链弹");
end
...
cls_player_liandan.deal_result = function(self, tbResult)
	...
	for idx = 1, #lst_buff do
		...
		if buff.scope ~= "LAST_TARGET" then
			for _, target in ipairs(targets) do
				cls_skill:end_display_call_back(ship, target, idx, nil, true)
			end
		end
	end
end